...
model = load_spiking_model("models/spike_n4_alternating_T50_seed0.pt",
                            n_classes=4, channels=10, T=50, mode="alternating", device=device)
x = x.to(device)

plot_layerwise_similarity(model, x, angles=[15,30,45,60,75,90],
                           metric="cka", plot_out="plots/alternating_layerwise_cka.png")

all_reps = rotation_sweep_representations(model, x, angles=list(range(0,360,15)))
R = rotation_sensitivity(all_reps)
plot_rotation_sensitivity_distribution(R, plot_out="plots/alternating_rotation_sensitivity.png")
